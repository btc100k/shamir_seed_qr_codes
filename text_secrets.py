...
# Print the big long string

# Decode
# collect series of big strings
# Split strings into shares.
# Combine with other strings & split
...
def do_encode():
    ...
    print("We are going to take your seed words, and create Shamir Secret Sharing JSON arrays for you to store.")
    print("-" * 20)
    ...
    print("Done.")
    print("We created {count} JSON snippets.".format(count=user_input.split_count))
    print("Print out these strings out and keep them in separate locations.")
    print("You'll need {count} strings to recover your seed words.".format(count=user_input.minimum_required))


def do_decode():
    print("We are going to restore your seed words from a series of JSON snippets.")
    print("Enter the strings one by one.")
    print("Use a blank line to indicate you're done adding shares.")
    print("-" * 20)

    segments = []
    for x in range(500):
        payload = input('Enter the JSON? ')
        if len(payload) > 0:
            try:
                ...
                print("Failed to load: {name}".format(name=payload))
        else:
            break
